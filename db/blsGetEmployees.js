...
let getData = async function(series, masterSeries) {
  const BLS_API_KEY = "05c41edacd064cce8b7a0aedeb7b1643";
  ...
  await bls.fetch(options).then(function(response) {
    ...
    for (let j = 0; j < cleanSeriesData.length; j++) {
      db.Employees.create({
        ...
        industry:
          cleanData[masterSeries.indexOf(cleanSeriesData[j].seriesID)]
            .industry_name,
        employees: cleanSeriesData[j].data[0].value
      });
    }
    fs.writeFile("employees.json", JSON.stringify(response), function(err) {
      if (err) throw err;
...
let getAllData = async function() {
  ...
  for (let j = 0; j < codes.length / 50; j++) {
    ...
    for (let k = j * 50; k < j * 50 + 50; k++) {
      ...
      let tempSeries = "CEU" + tempCode + "01";
      console.log(codes[j]);
